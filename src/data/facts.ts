...
// ─── Fact library ────────────────────────────────────────────
// The single source of geography and flag facts for the whole app. It has
// two layers built on the same curated trivia pool:
//
//   1. Fact of the Day - one curated trivia item surfaced per day, chosen
//      deterministically from the date so every player sees the same fact
//      on the same day (mirrors the daily challenge).
//   2. GeoFacts - the browse/quiz mode. It reuses the curated trivia pool
//      below and adds facts generated from the app's own datasets
//      (capitals, regions, land borders, landlocked, islands, twin flags),
//      so accuracy is grounded in existing data.
//
// ─── Curated trivia pool ─────────────────────────────────────
